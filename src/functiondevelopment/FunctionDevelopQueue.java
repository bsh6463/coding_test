...
public class FunctionDevelopQueue {
    ...
    public int[] solution(int[] progresses, int[] speeds) {
       ...
       int[] answer;
        ...
        for (int i=0; i<progresses.length; i++){
            functionQueue.add(new Function(progresses[i], speeds[i]));
        }
        ...
        for (int i=0; i<progresses.length; i++){
            if (i == 0){
                temp = functionQueue.remove().getRemain();
              cnt = 1;
            }
            else if (functionQueue.peek().getRemain() <= temp){
    ...
    public static void main(String[] args) {
        FunctionDevelopQueue functionDevelop = new FunctionDevelopQueue();
